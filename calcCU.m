...
%% Define Constants
% All formulas and tables taken from IES handbook page 10.41-10.42
zoneMultiplier = [  0,      0;...
                    0.041,  0.98;...
                    0.070,  1.05;...
                    0.100,  1.12;...
...
rhoCeil = 0;
rhoFloor = 0.2;
C1 = ((1-rhoWall)*(1-Fcc2fc.^2)*RCR)/((2.5*rhoWall*(1-Fcc2fc.^2))+(RCR*Fcc2fc*1-rhoWall)); % formula Taken from IES handbook page 10.42
C2 = ((1-rhoCeil)*(1+Fcc2fc))/(1+(rhoCeil*Fcc2fc)); 
C3 = ((1-rhoFloor)*(1+Fcc2fc))/(1+(rhoFloor*Fcc2fc));
C0 = C1 + C2 + C3;
CU = ((2.5*rhoWall*C1*C3*(1-Drcr)*propDown)/(RCR*(1-rhoWall)*(1-rhoFloor)*C0))+ ...
     ((rhoCeil*C2*C3*propUp)/((1-rhoCeil)*(1-rhoFloor)*C0))+...
((1-((rhoFloor*C3*(C1+C2))/((1-rhoFloor)*C0)))*((Drcr*propDown)/(1-rhoFloor)));
end